...
$fecha=date('Y-m-d');
	...
	if(isset($_REQUEST['del']) && $_REQUEST['del'] != 0)
	{
		...
		$sql_actividad = mysql_query("SELECT nombre, id_tipo_actividad FROM sys_actividades WHERE id_sys_actividades = ".$_REQUEST['del']);
...
<p></p>
<br>
<h4>Estas seguro que deseas eliminar la actividad <?php echo $fetch_actividad[0]; ?> ? <h4>
<h5>Esta acción no podrá reversarse en el futuro.</h5>
<br>
<br>
<div align="center">
<a href="configuraciones_maestras.php?action=del&form=actividades&actividad=<?php echo $actividad; ?>&msj=Actividad <?php echo $fetch_actividad[0]; ?> Eliminado Satisfactoriamente" type="button" class="btn btn-w-m btn-danger"><i class="fa fa-trash"></i> Eliminar</a> <button type="button" class="btn btn-w-m btn-default" data-dismiss="modal"> Cerrar</button>
</div>