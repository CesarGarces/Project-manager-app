...
$fecha=date('Y-m-d');
	...
	if(isset($_REQUEST['del']) && $_REQUEST['del'] != 0)
	{
		...
		$sql_dependencia = mysql_query("SELECT * FROM sys_dependencias WHERE id_dependencia = ".$_REQUEST['del']);
...
?>
<h3>Eliminar Dependencia </h3>
<p></p>
<br>
<h4>Estas seguro que deseas eliminar la dependencia <?php echo $fetch_dependencias[1]; ?> ? <h4>
<h5>Esta acción no podrá reversarse en el futuro.</h5>
<br>
<br>
<div align="center">
<a href="configuraciones_maestras.php?action=del&form=dependencias&dependencia=<?php echo $dependencia; ?>&msj=dependencia <?php echo $fetch_dependencias[1]; ?> Eliminada Satisfactoriamente" type="button" class="btn btn-w-m btn-danger"><i class="fa fa-trash"></i> Eliminar</a> <button type="button" class="btn btn-w-m btn-default" data-dismiss="modal"> Cerrar</button>
</div>